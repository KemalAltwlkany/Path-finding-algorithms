...
bool operator < ( const DSKey& key1, const DSKey& key2){ // compares two instances of class DSKey
    ...
    else if ( (std::abs(key1.k1 - key2.k1) < eps ) &&  (key1.k2 - key2.k2 < 0) ) return true;
    return false;
}
...
//This function directly defines the heuristic used. Euclidian distance (non-weighted)
DSKey DstarLite::Calculate_DSKey( Node* &s ){
    double pom1,pom2;
    if ( s->gvalue==inf && s->rhs==inf ) return DSKey (inf,inf);
    pom1=std::min(s->gvalue,s->rhs) + std::sqrt( (start_x - s->x)*(start_x - s->x ) + (start_y - s->y)*(start_y - s->y) ) + km;
    pom2=std::min(s->gvalue,s->rhs);
    return DSKey (pom1,pom2);
}

void DstarLite::Update_vertex_DS( Node* &u ){
    if( (*u)!=(*goalptr) ){
        Get_Succ(u);
        double minim, pominim;
        minim= (*Succ.begin())->gvalue + Compute_cost(u, (*Succ.begin()) );
        for( std::list<Node*>::iterator it=Succ.begin(); it!=Succ.end(); it++){
            pominim= (*it)->gvalue + Compute_cost(u, (*it));
            if ( pominim - minim < 0 ){
                minim=pominim;
            }
        }
    ...
    if ( u->in_open ){
        u->in_open=false;
        Erase_from_openlist(u);
    }
    if ( std::abs(u->gvalue - u->rhs) > eps ){
        u->kold=Calculate_DSKey(u);
        u->in_open=true;
        open_list.insert( std::upper_bound(open_list.begin(), open_list.end(), u, Compare_DS), u);
    }
}

void DstarLite::Update_vertex( Node* &s, Node* &sp ){} //had to be overriden, not used by D*Lite

void DstarLite::Get_Pred( Node* &s ){
    Update_neighbours(s);
    Pred=neighbours;
    neighbours.clear();
}

void DstarLite::Get_Succ( Node* &s ){
    Update_neighbours(s);
    Succ=neighbours;
    neighbours.clear();
}

void DstarLite::Compute_shortest_path_DS(){
    int br(1);
    Node* u;
    while (  ((*open_list.begin())->kold < Calculate_DSKey(startptr)) || ( startptr->rhs != startptr->gvalue)  ){
        if ( open_list.empty()){
            //21.06.2018.
            break;
        }
        ...
        if ( u->kold < Calculate_DSKey(u) ){
            u->kold = Calculate_DSKey(u);
            u->in_open=true;
            open_list.insert( std::upper_bound(open_list.begin(), open_list.end(), u, Compare_DS), u);
        }
        else if ( u->gvalue - u->rhs > 0 ){
            u->gvalue=u->rhs;
            Get_Pred(u);
            int brojac(1);
            for( std::list<Node*>::iterator it=Pred.begin(); it!=Pred.end(); it++){
                Update_vertex_DS((*it));
            }
        }
        else{
...
void DstarLite::Compute_forever(){
    int changes(0);
    int oznaka(2);
    double nesto;
    //initialize
    ...
    km=0;
    goalptr->rhs=0;
    goalptr->kold=(*this).Calculate_DSKey(goalptr);
    goalptr->in_open=true;
    open_list.insert( std::upper_bound(open_list.begin(), open_list.end(), goalptr, Compare_DS), goalptr);
    Compute_shortest_path_DS();
    startptr->traverse_cost=oznaka;
    double minima, tempima;
    Node* temp1;
    Node* temp2;
    Print_mapp();
    while ( (*startptr)!=(*goalptr) ){
        if ( startptr->gvalue == inf ){
            std::cout << "There is no known path!" << std::endl;
            break;
        }
        Get_Succ(startptr);
        temp1=(*Succ.begin());
        minima=temp1->gvalue + Compute_cost(startptr,temp1);
        for(std::list<Node*>::iterator it=Succ.begin(); it!=Succ.end(); it++){
           temp2=(*it);
           tempima=temp2->gvalue + Compute_cost(startptr,temp2);
           if ( tempima - minima < 0){
               minima=tempima;
               temp1=temp2;
           }
       }
       //std::cout << "sacekaj" << std::endl;
       //std::cin >> nesto;10
       startptr=temp1;
       startptr->traverse_cost=oznaka;
       std::cout << "------------------------------------------" << std::endl;
       Print_mapp();
       // MODIFIKACIJA 21.06.2018.
       std::cout << "Did any changes occur? N-0, Y-any" << std::endl;
       std::cin >> changes;
       if (changes){
            oznaka++;
            km+= std::sqrt( (lastptr->x - startptr->x)*(lastptr->x - startptr->x) + (lastptr->y - startptr->y)*(lastptr->y - startptr->y)  );
            lastptr=startptr;
            int x,y;
            Node* changednode;
            std::list<Node*> changelog;
            while(1){
                std::cout << "Input x coordinate of changed Node, x=";
                std::cin >> x;
                std::cout << "Input y coordinate of change Node, y=";
                std::cin >> y;
                if ( x<0 || y<0 ){
                    break;
                }
                if (mapp[x][y].traversable){
                    mapp[x][y].traversable=false;
                    mapp[x][y].not_traversable=true;
                    mapp[x][y].traverse_cost=1;
                }
                else{
                    mapp[x][y].traversable=true;
                    mapp[x][y].not_traversable=false;
                    mapp[x][y].traverse_cost=0;
                }
                //mapp[x][y].traversable=mapp[x][y].not_traversable;
               // mapp[x][y].not_traversable=!mapp[x][y].traversable;
                changednode=&mapp[x][y];
                Update_neighbours(changednode);
                changelog=neighbours;
                neighbours.clear();
                Update_vertex_DS(changednode);
                for(std::list<Node*>::iterator it=changelog.begin(); it!=changelog.end(); it++){
                    Update_vertex_DS((*it));
                }
            }
            Compute_shortest_path_DS();
...
void DstarLite::Generate_mapp_DS(){
    int c;
    for(int i=0; i<m; i++){
        for(int j=0; j<n; j++){
            mapp[i][j].x=i;
